...
class SIIMISICDataset(Dataset):
    def __init__(self, csv,transform=None, transform2=None):
    ...
    def __getitem__(self, index):
        ...
        return data, torch.tensor(self.csv.iloc[index, self.csv.columns.get_loc("target")]).long()

class DimTransform:
    def __init__(self, target_dim, classes):
        self.target_dim = target_dim
        self.class_split = classes

    def __call__(self, x):
        
        label = torch.zeros(self.target_dim)
        label[self.class_split.index(np.argmax(x))] = 1
...
class ISIC(Dataset):
    def __init__(self, data_dir, label_dir, transform=None, target_transform=None):
        ...
        '''path = img_dir[:-1]
        self.path_train = os.path.join(os.path.dirname(path),"train/")
        self.path_val = os.path.join(os.path.dirname(path),"val/")
        self.path_ood = os.path.join(os.path.dirname(path),"ood/")
        self.loaded = False'''
        ...
        self.label_transform = DimTransform(len(self.classes), self.classes)
        self.data = []
        self.label = []
        self.load_data()
    ...
    def load_data(self):
        ...
        if os.path.isfile(self.label_dir):  
        ...
        with tqdm.tqdm(total=len(os.listdir(self.data_dir)), ncols=100, mininterval = 5) as _tqdm:
...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None):
    ...
    if dataset == 'CIFAR10':
        ...
        transform = transforms.Compose([
                transforms.ToPILImage(), transforms.Resize(im_size), transforms.ToTensor(),
                transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
            ])
        img_dir = data_path + "/ISIC_2019_Training_Input" 
        label_dir = data_path + "/ISIC_2019_Training_GroundTruth.csv"
        
        dst_train =  ISIC(img_dir, label_dir, transform=transform)# no augmentation

        torch.manual_seed(10)
        dst_train, dst_test = torch.utils.data.random_split(dst_train, [0.8, 0.2])
        dst_train = dst_train.dataset
        dst_test = dst_test.dataset
        ...
        img_dir = data_path + "/dataset_images" 
        train_label_dir = data_path + "/train_score.csv"
        test_label_dir = data_path + "/all_test.csv"

        dst_train =  Fitzpatrick17k(img_dir, train_label_dir, transform=transform)# no augmentation
        dst_test = Fitzpatrick17k(img_dir, test_label_dir, transform=transform)
