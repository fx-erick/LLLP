...
from dc_utils import get_network, get_eval_pool, evaluate_synset, get_time, epoch, DiffAugment, ParamDiffAug, number_sign_augment, parser_bool, downscale, DECE
from others_edc import DecoyDataset
...
from utils_edc import w2_distance, ECEMeter
...
def main():
    ...
    parser.add_argument('--ece_weight', type=float, default=10, help='outer loop for network update')
    parser.add_argument('--w2_weight', type=float, default=1e-3, help='w2 regularization weight')
    ...
    parser_bool(parser, 'eval_laplace', False)
    ...
    args.save_path = os.path.join('result', '%s_LPC_%s_ipc%d'%(args.dataset, args.method, args.ipc), 
    ...
    for exp in range(args.num_exp):
        ...
        for it in range(args.Iteration+1):
            ...
            if it in eval_it_pool[1:]:
                for model_eval in model_eval_pool:
                    ...
                    for it_eval in range(args.num_eval):
                        ...
                        _, acc_train, acc_test, nll_train, nll_test, ece_train, ece_test, brier_train, brier_test = evaluate_synset(it_eval, net_eval, image_syn_eval, label_syn_eval, testloader, args)
                        accs.append(acc_test)
                        nlls.append(nll_test)
                        eces.append(ece_test)
                        briers.append(brier_test)
                    print('Evaluate %d random %s, mean = %.4f std = %.4f\n-------------------------'%(len(accs), model_eval, np.mean(accs), np.std(accs)))
                    print('NLL = %.4f, ECE = %.4f, Brier = %.4f '%(np.mean(nlls), np.mean(eces), np.mean(briers)))
                ...
                for acc_index, acc_meter in enumerate(acc_meters):
                    ...
                    acc_meter.reset()
                for _, ece_meter in enumerate(ece_meters):
                    ece_meter.reset()
                acc_report += ' Testing Acc: '
                nll_report = " Testing NLL: "
                ece_report = " Testing ECE: "
                brier_report = " Testing Brier: "
                for net_index, test_model in enumerate(net_list):
                    loss_test, acc_test, o_test, brier_test  = epoch('test', testloader, test_model, None, criterion, args, aug = False)
                    acc_report += 'Net {}: {:.2f}% '.format(net_index, acc_test*100)
                    nll_report +=  'Net {}: {:.2f}% '.format(net_index, loss_test )
                    ece_report +=  'Net {}: {:.2f}% '.format(net_index, ece_test )
                    brier_report +=  'Net {}: {:.2f}% '.format(net_index, brier_test )
                print(acc_report)
                print(nll_report)
                print(ece_report)
                print(brier_report)
            ...
            train_ece_list = [ece_meters[ind] for ind in net_index_list]
            ...
            for _ in range(args.outer_loop):
                ...
                if 'BN' not in args.model or args.model=='ConvNet_GBN': # for ConvNet
                    for image_sign, image_temp in [['syn', image_syn]]:
                        ...
                        for net_ind in range(len(train_net_list)):
                            ...
                            net_ece = train_ece_list[net_ind]
                            for c in range(num_classes):
                                ...
                                lab_real = torch.ones(img_real.shape[0])*c
                                ...
                                ds_real = DecoyDataset(img_real, lab_real.type(torch.LongTensor))
                                ds_syn = DecoyDataset(img_syn, lab_syn.type(torch.LongTensor))

                                dl_real = DataLoader(ds_real, batch_size = 64, shuffle=True )
                                dl_syn = DataLoader(ds_syn, batch_size = args.ipc, shuffle = True)
                                ...
                                if args.ipc == 1 and not args.aug:
                                    ...
                                    indices_topk_loss = torch.topk(loss_real, k=8, largest=False)[1]
                                ...
                                syn_ce_loss = 0
                                ace_ce_loss = DECE(args.device)
                                if args.syn_ce:
                                    weight_i = net_acc.value()[0] if net_acc.n != 0 else 0
                                    weight_e = net_ece.value() if net_ece.n != 0 else 0
                                    ...
                                    if args.ipc == 1 and not args.aug:
                                        ...
                                        syn_ce_loss += (F.cross_entropy(logits_syn, lab_syn.repeat(args.aug_num)) * weight_i)
                                        syn_ce_loss += (ace_ce_loss(logits_syn, lab_syn.repeat(args.aug_num)) * weight_i*args.ece_weight)


                                    loss_c += (syn_ce_loss * args.ce_weight)
                                
                                la_model_real = Laplace(copy.deepcopy(net),"classification",subset_of_weights="last_layer", hessian_structure="diag")
                                la_model_real.fit(dl_real)
                                la_model_real.optimize_prior_precision()
                                f_mu_real, f_var_real = la_model_real._glm_predictive_distribution(img_real[:args.ipc])

                                la_model_syn = Laplace(copy.deepcopy(net),"classification", subset_of_weights="last_layer", hessian_structure="diag")
                                la_model_syn.fit(dl_syn)
                                la_model_syn.optimize_prior_precision()
                                f_mu_syn , f_var_syn = la_model_syn._glm_predictive_distribution(img_syn)

                                W2_loss = w2_distance(f_mu_real, f_var_real, f_mu_syn, f_var_syn)
                                loss_c += ( W2_loss * args.w2_weight) 
                ...
                for j in range(min(args.fetch_net_num, len(shuffled_net_index))):
                    ...
                    ece_meter_net_train = ece_meters[training_net_idx]
                    for i in range(args.model_train_steps):
                        ...
                        ece_meter_net_train.add(real_logit.detach(), lab_real_)
            ...
            if it%100 == 0: # only record the final results
