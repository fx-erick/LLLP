...
import torchnet.meter.meter as meter
import numbers
matplotlib.use('Agg')
...
def w2_distance(mean1, cov1, mean2, cov2):
        cov1 = torch.linalg.cholesky(cov1)
        cov2 = torch.linalg.cholesky(cov2)
        
        mean_diff = mean1 - mean2
        ret = torch.norm(mean_diff, p=2) ** 2

        
        sqrt_cov1 = torch.sqrt(cov1)
        sqrt_cov2 = torch.sqrt(cov2)
        cov_mean = torch.sqrt(sqrt_cov1 @ cov2 @ sqrt_cov1)
    

        ret = ret + trace( cov1 + cov2 - 2*cov_mean)
        return ret

def trace(cov):
    return cov.diagonal(offset=0, dim1=-1, dim2=-2).sum(-1).sum(-1)

class ECEMeter(meter.Meter):
    def __init__(self):
        super(ECEMeter, self).__init__()
        self.reset()
