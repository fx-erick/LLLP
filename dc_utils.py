...
class DECE(nn.Module):
    ...
    def __init__(self, device, num_bins = 20, t_a = 100, t_b =0.01 ):
...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug, step=-1, bsr=False, pooling=False, partial=None):
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        lab_onehot = F.one_hot(lab, num_classes=8)
        brier = ((F.softmax(output, dim=-1) - lab_onehot)**2).sum(1).mean()
    ...
    return loss_avg, acc_avg, o, avg_brier
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args, upsample=False, pooling=False, partial=None, aug=True, tqdm_bar=False):
    ...
    for ep in training_range:
        if not upsample:
            loss_train, acc_train, o_train, brier_train = epoch('train', trainloader, net, optimizer, criterion, args, aug = aug, pooling=pooling)
        else:
        ...
        if tqdm_bar and 'imagenet_subset' not in args.dataset:
            ...
            loss_test, acc_test, o_test, brier_test = epoch('test', testloader, net, optimizer, criterion, args, aug = False, partial=partial)
            print('%s Evaluate_%02d: epoch = %04d train time = %d s train loss = %.6f train acc = %.4f, test acc = %.4f' % (get_time(), it_eval, Epoch, int(time_train), loss_train, acc_train, acc_test))
            print(' train ece = %.4f train brier = %.4f , test nll = %.4f test ece = %.4f, test brier = %.4f' % ( o_train, brier_train, loss_test, o_test, brier_test))

    time_train = time.time() - start
    if args.eval_laplace:
        net = Laplace(net,"classification",subset_of_weights="last_layer", hessian_structure="diag")
        net.fit(trainloader)
        net.optimize_prior_precision()
    loss_test, acc_test, o_test, brier_test = epoch('test', testloader, net, optimizer, criterion, args, aug = False, partial=partial)
    print('%s Evaluate_%02d: epoch = %04d train time = %d s train loss = %.6f train acc = %.4f, test acc = %.4f' % (get_time(), it_eval, Epoch, int(time_train), loss_train, acc_train, acc_test))
    print(' train ece = %.4f train brier = %.4f , test nll = %.4f test ece = %.4f, test brier = %.4f' % ( o_train, brier_train, loss_test, o_test, brier_test))

    return net, acc_train, acc_test, loss_train, loss_test, o_train, o_test, brier_train, brier_test
...
def evaluate_synset_mixup(it_eval, net, images_train, labels_train, testloader, args, num_classes):
    ...
    time_train = time.time() - start
    loss_test, acc_test, o_test, brier_test = epoch('test', testloader, net, optimizer, criterion, args, aug = False)
    print('%s Evaluate_%02d: epoch = %04d train time = %d s train loss = %.6f train acc = %.4f, test acc = %.4f' % (get_time(), it_eval, Epoch, int(time_train), loss_train, acc_train, acc_test))
